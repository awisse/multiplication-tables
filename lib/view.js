...
import {
  ...
  STAR_PNG, HPCT_PNG, PASS_SND, FAIL_SND, APPLAUSE_SND, CHEERING_SND,
  WARNING_SND, PLAYERS_JSON, NAMES_PAGE, QUIZ_PAGE, RESULT_PAGE
} from "./constants.js"
...
class View {
  ...
  moveStarTo (ix, hundredPctRunning) {
     ...
     * moving star. From current position of star:
  ...
  #show100pct () {
    ...
    // Need a timeout directly after an `unhide`. See:
...
function infoDialog (message) {
  /* Inform the user with a modal dialog */
...
function confirmDialog (message, yesCallback, noCallback) {
   ...
   * The `yesCallback` is called if the user chooses the "Yes" button,
   * the `noCallback` is called if the user chooses the "No" button. */
  const dialog = createElement("dialog")
  dialog.innerText = message
  const form = createElement("form", "dialog-form")
  form.method = "dialog"
  ...
  const yesButton = createElement("button", "dialog-button")
  const noButton = createElement("button", "dialog-button")
  yesButton.innerText = "Yes"
  noButton.innerText = "No"
  buttonBox.append(noButton, yesButton)
  form.append(buttonBox)
  dialog.append(form)
  document.getElementById("gameboard").append(dialog)
  /* Event Listeners */
  if (yesCallback) yesButton.addEventListener("click", yesCallback)
  if (noCallback) noButton.addEventListener("click", noCallback)
  dialog.showModal()
}
