 ...
 * 2. Playing sounds
 */
import localizedStrings from "../locale/default.js"
import {
  LANG_EV, ADD_PLAYER_EV, PLAY_DELETE_EV, ANSWER_EV, RESTART_EV, SAVE_EV,
  KEY_DOWN_EV, KEY_UP_EV, LOAD_EV, CHANGE_PLAYER_EV, PLAY, DELETE,
  MOVE_STAR_DELAY, PLOT_WIDTH, PLOT_HEIGHT, STAR_SIZE, BIG_STAR_SIZE,
  IMG_PATH, STAR_PNG, HPCT_PNG, PLAYERS_JSON, NAMES_PAGE, QUIZ_PAGE,
  RESULT_PAGE
} from "./constants.js"
...
// Holds message strings in localized language
let locale = localizedStrings.FR
...
class View {
  ...
  constructor (title) {
    ...
    this.#header = getElement("header.page-header")
    ...
    this.pageTitle = createElement("h1", "page-title")
    this.pageTitle.id = "page-title"
  ...
  #createMenus () {
    ...
    this.langMenu = document.createElement("select")
    this.langMenu.name = "language"
    this.langMenu.id = "lang-select"

    // Add languages
    let langSelect
    for (const lang of Object.keys(localizedStrings)) {
      langSelect = document.createElement("option")
      langSelect.innerText = lang
      langSelect.value = lang
      this.langMenu.append(langSelect)
    }

    this.langMenu.addEventListener("change", this.#handlers[LANG_EV])
    this.#header.prepend(this.langMenu)
  }
  ...
  showNamesPage () {
    ...
    hide(this.footer)
    unhide(this.langMenu)

    // Language adjustment of UI Elements
    ...
    this.langMenu.value = this.#language
    ...
    // Title of names page
    this.pageTitle.textContent = locale.usersAndScores
    this.main.append(this.pageTitle)
  ...
  showQuizPage (name) {
    ...
    // The question
    this.pageTitle.textContent = `${name}, ${locale.howMuchIs}`
    this.pageTitle.append(this.quizProblemDisplay, "?")
    this.main.append(this.pageTitle)
  ...
  #setupGameOverPage () {
    ...
    /* The button box at the bottom of the Game Over page */
    this.restartButtons = createElement("div", "buttonbox")

    // The Restart Button. Restarts the game for the same player on the
    // Quiz page.
    this.restartButton = createElement("button")
    ...
    // The "Change Player" button. Restarts the game from the Name page.
    const changePlayerButton = createElement("button")
    changePlayerButton.textContent = locale.changePlayer
    changePlayerButton.addEventListener("click",
      this.#handlers[CHANGE_PLAYER_EV])

    this.restartButtons.append(this.restartButton, changePlayerButton)
  }
  ...
  showGameOverPage (name, score, percentage, results) {
    ...
    this.main.append(this.restartButtons)
  ...
  setLanguage (language) {
    ...
    locale = localizedStrings[language]
  }
  ...
  showDialog (type, message, callback) {
     ...
     *    pressed. */
    let dialog
    const nyButtons = [locale.no, locale.yes]
    ...
    switch (type) {
      ...
      case "confirm":
        ...
        dialog = new ConfirmDialog(message, this.main, callback, nyButtons)
        break
      default:
