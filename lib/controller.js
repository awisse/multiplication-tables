...
import {MIN_STAR_PCT} from "./constants.js"
...
class Controller {
  ...
  #handleAnswer(event) {
    ...
    if (this.quiz.checkAnswer(parseInt(clicked.value, 10))) {
      ...
      const timeout = setTimeout(this.#newProblem.bind(this), ANSWER_DELAY);
    } else {
      this.#handleFail();
    }
  ...
  #gameOver = (name, score, percentage) => {
    ...
    // TODO: Only show high score star if percentage < 60%
    ...
    const starHS = prevHS.ix === undefined ? currentHS : prevHS;
    if (starHS.note >= MIN_STAR_PCT) this.view.showStarAt(starHS.ix);
