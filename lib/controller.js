 ...
 * */
import {
  TESTING, FAIL, PLAY, DELETE, MAX_COMBINATIONS, LANG_EV, ADD_PLAYER_EV,
  PLAY_DELETE_EV, ANSWER_EV, RESTART_EV, SAVE_EV, KEY_DOWN_EV, KEY_UP_EV,
  LOAD_EV, PLAYERS_CHANGED_EV, CHANGE_PLAYER_EV, ANSWER_DELAY, MIN_STAR_PCT,
  NAMES_PAGE, DEFAULT_LANGUAGE
} from "./constants.js"
import { Quiz, loadLanguage, saveLanguage } from "./model.js"
...
class Controller {
  constructor (players, view) {
    ...
    this.view.addHandler(LANG_EV, this.#languageSelect.bind(this))
    ...
    this.view.addHandler(CHANGE_PLAYER_EV, this.start.bind(this))
    this.view.addHandler(RESTART_EV, this.#play.bind(this))

    this.view.setupListeners()

    // Set UI Language
    const lang = loadLanguage() || DEFAULT_LANGUAGE
  ...
  #languageSelect (e) {
    /* Player has selected a language in the menu */
  ...
  #handleKeyUp (event) {
    ...
    if ((event.code === "AltLeft") && (this.players.players.length > 0)) {
  ...
  #handleImportError (error) {
    ...
    let message = locale[cause.msg] || cause.msg
    message += cause.value ? ` : "${cause.value}"` : ""
  ...
  #deleteAllPlayers () {
    ...
    function deleteAll (answer) {
      if (answer === locale.yes) {
        ...
        this.view.showDialog("warning", locale.allDeleted)
      }
    }
    this.view.showDialog("confirm", locale.confirmDeleteAll,
      deleteAll.bind(this))
  }
